...
SYSTEM_INFO = '''
You are a data quality assessment expert specializing in evaluating the quality of instruction answers.'''


PROMPT = '''
Evaluate the provided instruction and its corresponding output based on the following criteria, and structure your response as outlined below:

Evaluation Criteria:​
1. ​​​​Relevance:​​ Does the output directly address and fulfill the instruction's request?
2. Correctness:​​ Is the output factually accurate, and are its reasoning steps (if any) sound and free from errors?
3. Logical Soundness:​​ Is the output logically consistent, well-structured, and free from contradictions or significant gaps?
...
3.Highlight any specific logical flaws, inconsistencies, or unmet conditions, if present.
...
def main():
    ...
    for item in data:
        ...
        formatted_template=formatted_template.replace('{Output}',item["conversations"][1]["value"] if item["conversations"][0]["from"] == "human" else item["conversations"][2]["value"])
    ...
    for i, output in enumerate(outputs):
            output_data.append({
                ...
                "instruction_score_origin_A":output.outputs[0].text,
                "instruction_score_A": str(remove_boxed(last_boxed_only_string(output.outputs[0].text))),
                "instruction_score_origin_Q":data[i].get("instruction_score_origin_Q","null"),
                "instruction_score_Q": data[i].get("instruction_score_Q","null"),
            })
